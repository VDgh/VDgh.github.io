...
let Cell ;
 function showStructure(struScene, struType, animGauge, pz, scale = 1){
    ...
    if(allThreeStructures.children.length <= struType){
        ...
        structuteGroup.children.push(new THREE.Group());
        structuteGroup.children.push(new THREE.Group());
        structuteGroup.children.push(new THREE.Group());
        ...
        SiSph = new THREE.Mesh( SiGeometry, SiMaterial ); 
        ...
        OxiSph = new THREE.Mesh( OxiGeometry, OxiMaterial );      
        ...
        if(struType == cellType) { 
            ...
            structuteGroup.children.push(Cell);
        }
    ...
    siPos = []; siPs = new THREE.Vector3();
      for(let i1 = 0; i1<struSi[0].length; i1++){
        siPs.x = pz.x + (struSi[lBnd][i1][0] * (1 - wt) + wt * struSi[rBnd][i1][0]) * scale;
        siPs.y = pz.y + (struSi[lBnd][i1][2] * (1 - wt) + wt * struSi[rBnd][i1][2]) * scale;
        siPs.z = pz.z + (struSi[lBnd][i1][1] * (1 - wt) + wt * struSi[rBnd][i1][1]) * scale;
        if(siGroup.children.length<= i1) siGroup.children.push(SiSph.clone());
        siGroup.children[i1].position.copy(siPs);
        siPos.push(siPs.clone());
    }   
   
    oxiPos = [];oxiPs = new THREE.Vector3();
    for(let i1 = 0; i1<struOxi[0].length; i1++){
        oxiPs.x = pz.x + (struOxi[lBnd][i1][0] * (1 - wt) + wt * struOxi[rBnd][i1][0]) * scale;
        oxiPs.y = pz.y + (struOxi[lBnd][i1][2] * (1 - wt) + wt * struOxi[rBnd][i1][2]) * scale;
        oxiPs.z = pz.z + (struOxi[lBnd][i1][1] * (1 - wt) + wt * struOxi[rBnd][i1][1]) * scale;
        if(oxiGroup.children.length<= i1) oxiGroup.children.push(OxiSph.clone());
        oxiGroup.children[i1].position.copy(oxiPs);
        oxiPos.push(oxiPs.clone());
    }   
    ...
    for(let i1 = 0; i1<struBond[0].length; i1++){ 
        ...
        for(let i2 = 1; i2<struBond[0][i1].length; i2++){ 
            ...
            oxiVec =oxiPos[oNm];
            bNb++;
            if(bondsGroup.children.length<= bNb){
                var direction = new THREE.Vector3().subVectors(siVec, oxiVec);
                var bondsMaterial = new THREE.MeshBasicMaterial({ color: 0x5B5B5B });
                var bondsGeometry = new THREE.CylinderGeometry(0.07 * scale, 0.07 * scale, direction.length(), 6, 4, false);
                ...
                bondsGroup.children.push(bond);
            }
            bondsGroup.children[bNb].position.copy(siVec); 
            bondsGroup.children[bNb].lookAt(oxiVec);
        }
    }
