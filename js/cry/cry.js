...
function placeStucture(sceneIntro,animGauge){
   ...
   sceneIntro.remove(cellGroup);
   cellGroup = showStructure(cellType, animGauge);
   cellGroup.position.copy(scnIntroCenter);
   cellGroup.position.x += 1.5;
   cellGroup.position.z += 1.0;
   cellGroup.scale.set(.2,.2,.2);
   sceneIntro.add(cellGroup);

   sceneIntro.remove(tetraGroup);
   tetraGroup = showStructure(tetraType, animGauge);
   tetraGroup.position.copy(scnIntroCenter);
   tetraGroup.position.x += 1.2;
   tetraGroup.position.z += 2.5;
   tetraGroup.scale.set(.3,.3,.3);
   sceneIntro.add(tetraGroup);

   sceneIntro.remove(bulkGroup);
   bulkGroup = showStructure(bulkType, animGauge);
   bulkGroup.position.copy(scnIntroCenter);
   bulkGroup.position.x += 1.2;
   bulkGroup.position.z -= 1.5;
   bulkGroup.scale.set(.15,.15,.15);
