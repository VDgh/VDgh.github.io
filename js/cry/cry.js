...
function setShapeFace(qzCr, codeName, dic){
...
function sceneIntro( sceneIntro,  qrz = null){
  ...
  if(qrz != null){
   ...
   let overTheFace ={};
   scnIntroCenter = subScene[0].position;
   crPack = setShapeFace(qrz,codeName,overTheFace);
   //crPack[2].scale.set(.03,.03,.03);
   crPack[2].position.copy(sceneIntro.position);
   crPack[2].position.x+=2
   crPack[2].position.y-=4
   crPack[2].position.z-=4
   sceneIntro.add(crPack[2]);
   crystals.push(crPack);
  //--------------------------------------------------------
  // rayList = crPack[2].children;
 //--------------------------------------------------------
...
function placeStucture(sceneIntro,animGauge){
   if(psStru.length<1){
      psStru.push(new THREE.Vector3());
      psStru[0].copy(sceneIntro.position);
      psStru[0].x += 6;
      psStru[0].y -= 5;
      psStru[0].z -= 1;
   }
   showStructure(sceneIntro, tetraType, animGauge, psStru[0],.3);
   if(psStru.length<2){
      psStru.push(new THREE.Vector3());
      psStru[1].copy(sceneIntro.position);
      psStru[1].x += 6;
      psStru[1].y -= 5;
      psStru[1].z -= 3;
   }
   showStructure(sceneIntro, cellType, animGauge, psStru[1], .2);
   if(psStru.length<3){
      psStru.push(new THREE.Vector3());
      psStru[2].copy(sceneIntro.position);
      psStru[2].x += 6;
      psStru[2].y -= 5;
      psStru[2].z -= 6;
   }
   showStructure(sceneIntro, bulkType, animGauge, psStru[2],.15);
   ...
   if(qzCr != null){
     ...
     // oriPak = []; oriPak.push(qrz);
      ...
      for(let c = 0;c < cutQz.length; c++){
         ...
         scnCutCenter = subScene[1].position;;
