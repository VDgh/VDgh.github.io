   ...
   // inp.addEventListener("change", updateValue);
    ...
    let  vectorPoint = class{ constructor(){} coo = [3]; };
 
    let  planeEq = class{ constructor(){}  eq = [4]; };
            
    let  lineVertices = class{ constructor(){} vx = [2]; };
    ...
    let  grPlaneChar = {
                        'nameCode' : [1],                           // kod na imeto na ploskostta
                        'specialName' : [1],                        // kod na imeto na ploskostta
    ...
    let zoneChar = class{ constructor(){} 
                        ...
                        gravityC = new vectorPoint();    // cent@r na tejest
...
function loadCrystal(crData){
    ...
    for (let  i = 0; i < qrz.geometry.nmbPlanes_n; i++)
    {
        ...
        qrz.geometry.pl_n[i].gravityC = new vectorPoint();
        for (let j = 0; j < 3; j++)
        {
            qrz.geometry.pl_n[i].gravityC.coo[j] = getDouble();
        }
   ...
   {
        qrz.geometry.p_n[i] = new vectorPoint();
        qrz.geometry.p_r[i] = new vectorPoint();
        for (let j = 0; j < 3; j++)
        {
            dbl = getDouble();
            qrz.geometry.p_n[i].coo[j] = dbl;
            qrz.geometry.p_r[i].coo[j] = dbl;
        }
     }



//alert("P coo  " +  qrz.geometry.p_n[qrz.geometry.nmbPoints_n-1].coo[2] + " \n " + pz + "  " + crData.length);
    ...
    for (let i = 0; i < qrz.geometry.nmbZones_n; i++)
    {
        ...
        qrz.geometry.zone_n[i].gravityC =new vectorPoint();
        qrz.geometry.zone_n[i].gravityC = new vectorPoint();
        for (let j = 0; j < 3; j++)
        {
            qrz.geometry.zone_n[i].gravityC.coo[j] = getDouble();
        }
        intVal = getInteger();
    ...
    for(let zn = 0; zn< 2; zn++){
        ...
        for(let i = 0; i< qrz.geometry.zone_n[zn].nmbPlanes; i++){
            ...
            const out = new THREE.Vector3(vOut[0],vOut[1],vOut[2]);
                ...
                qrz.geometry.pl_n[pNm].pPolygon= [qrz.geometry.pl_n[pNm].nmbLines];
             for( let j = 0; j<qrz.geometry.pl_n[pNm].nmbLines; j++){
                ln =  qrz.geometry.pl_n[pNm].pLine[j];
                if(j==0){
                    if(ln<0) qrz.geometry.pl_n[pNm].pPolygon[j] = qrz.geometry.li_n[-ln].vx[1];
                    else  qrz.geometry.pl_n[pNm].pPolygon[j] = qrz.geometry.li_n[ln].vx[0];
                }
                else{
                    ln=Math.abs(ln);
            ...
            v1 =[3];v2 =[3];
            for(let j = 0; j < 3; j ++){
                v1[j] = qrz.geometry.p_n[qrz.geometry.pl_n[pNm].pPolygon[1]].coo[j] -qrz.geometry.p_n[qrz.geometry.pl_n[pNm].pPolygon[0]].coo[j];
                v2[j] = qrz.geometry.p_n[qrz.geometry.pl_n[pNm].pPolygon[2]].coo[j] -qrz.geometry.p_n[qrz.geometry.pl_n[pNm].pPolygon[1]].coo[j];
            ...
            polyAngl = Math.acos(out.dot(nVec));
            normAngl = Math.acos(out.dot(norm));
...
function  getInteger(){
    ...
    try{
     ...
     alert( bt.length + "   " + pz + "   " + cry.length);
      }
}

function  getShortInt(){
    bt = cry.slice(pz, pz+2).reverse();
    pz+=2;
    var buf = new Uint8Array(bt).buffer;
    var view = new DataView(buf);
