    ...
    let  vectorPoint = class{  
        ...
        add(v){ for (let i=0; i<3; i++) this.coo[i] += v.coo[i]; }
    };
   ...
   let qrz  = new quartzCrystal(); 
    ...
    for (let  i = 0; i < qrz.geometry.nmbPlanes_n; i++)
    {
        ...
        for (let j = 0; j < qrz.geometry.pl_n[i].nmbLines; j++)
        {
            ...
            if(qrz.geometry.pl_n[i].pLine[j] == -222)
            alert("-222  " + j + "  " + i);
            if(j==0 && i==41 ) alert('41  ' + qrz.geometry.pl_n[i].pLine[j])
            if(j==0 && i==46 ) alert('46  ' + qrz.geometry.pl_n[i].pLine[j])
            if(j==1 && i==74 ) alert('74  ' + qrz.geometry.pl_n[i].pLine[j])
...
function addPolygons(qrz){
    ...
    for(let zn = 0; zn<2; zn++){  // qrz.geometry.nmbZones_n; zn++){
