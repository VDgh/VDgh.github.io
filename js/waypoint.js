...
class Waypoint {
   ...
   deleteWp = function(){
	...
	bezierPath = function(draw,steps,wpBf,wpAf){
		...
		this.bzPoly.setMap(map);
		if(draw) this.bzPoly.setPath(pathBz);
	...
	setFromXml = function(xml){
			...
			if(el.length>0)this.id =parseInt(el[0].getAttribute('value'));
			...
			if(el.length>0)this.altitude =parseInt(el[0].getAttribute('value'));
			
			el= xml.getElementsByTagName('elevation');
			if(el.length>0)this.elevation =parseInt(el[0].getAttribute('value'));

			el= xml.getElementsByTagName('speed');
			if(el.length>0)this.speed =parseFloat(el[0].getAttribute('value'));

			el= xml.getElementsByTagName('headingId');
			if(el.length>0)this.headingId =parseInt(el[0].getAttribute('value'));

			el= xml.getElementsByTagName('azimuth');
			if(el.length>0)this.azimuth =parseInt(el[0].getAttribute('value'));
			
			el= xml.getElementsByTagName('pitchAngle');
			if(el.length>0)this.pitchAngle =parseInt(el[0].getAttribute('value'));

			el= xml.getElementsByTagName('smoothing');
			if(el.length>0)this.smoothing =parseFloat(el[0].getAttribute('value'));
