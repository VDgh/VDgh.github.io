...
if(	tiltRotBtns[0].style.visibility == 'hidden') return;
...
function slowDown(evn) {
	...
	elevator.getElevationForLocations({locations: [evn.latLng],},function(results, status) {reliefElevation = results[0].elevation;});
}
...
function redrawPath() {
    ...
    for (let i = 1; i < waypoints.length; i++) {
		...
		if(dis>10){pt=" M -6,3 6,3 6,-3 -6,-3  z ";}
		if(dis>100){pt=" M -7,3 7,3 7,-3 -7,-3  z ";}
		if(dis>1000){pt=" M -8,3 8,3 8,-3 -8,-3  z ";}
		const lbl = {
			path: pt,
			fillColor: "#300",
			fillOpacity: 1.0,
